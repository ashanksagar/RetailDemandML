...
Production-style retail demand forecasting platform for daily store/item sales. This is built as an ML engineering project, not a notebook: it includes data validation, leakage-aware feature engineering, model comparison, rolling backtests, MLflow tracking, model selection, batch forecasting, explainability hooks, FastAPI serving, Docker, CI, linting, typing, and tests.
...
- Time-aware forecasting workflow with chronological splits and rolling-origin backtests.
- Multiple baselines and ML candidates instead of a single model demo.
- Data contracts, metadata, quality checks, and Pandera schema support.
- Training-serving skew control through a saved feature-state artifact.
- Business-style API accepts `store`, `item`, and `forecast_date`; it does not require callers to hand-build lag features.
- Residual-based prediction intervals in batch test predictions.
- Sliced evaluation by volume bucket and store.
- Model card, model comparison table, backtest report, and MLflow artifacts.
- CI-ready engineering loop: `ruff`, `mypy`, `pytest`.
